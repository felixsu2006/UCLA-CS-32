...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
	...
	{
		Store store = s[unsorted];
		int loc = unsorted;
		while ((loc > 0) && comp(store, s[loc-1]))
		{
			s[loc] = s[loc - 1];
			loc--;
		}
...
int main()
{
	...
	// destruction time.
	{
		...
		vector<Store*> m_stores;
		for (int k = 0; k < auxStores.size(); k++)
		{
			m_stores.push_back(&auxStores[k]);
		}
		// TODO:  Sort the vector of pointers using the STL sort algorithm
		//        with compareStorePtr as the ordering relationship.
		sort(m_stores.begin(), m_stores.end(), compareStorePtr);
